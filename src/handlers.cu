...
FunctionMapping functionMappings[] = {
  // 4.11 Initialization and Cleanup
...
void *getFunctionByName(const char *name) {
    for (int i = 0; i < sizeof(functionMappings) / sizeof(FunctionMapping); i++) {
        if (strcmp(functionMappings[i].name, name) == 0) {
            return functionMappings[i].function; 
        }
    }
    return NULL;  // Return NULL if the function name is not found
}