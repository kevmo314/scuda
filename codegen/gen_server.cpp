...
int handle_cublasSgemm_v2(void *conn)
{
    ...
    if (rpc_read(conn, &k, sizeof(int)) < 0)
        goto ERROR_0;
        if (rpc_read(conn, (void*)alpha, sizeof(const float*)) < 0)
    if (rpc_read(conn, &alpha, sizeof(const float*)) < 0)
        goto ERROR_0;
        if (rpc_read(conn, (void*)A, sizeof(const float*)) < 0)
    if (rpc_read(conn, &A, sizeof(const float*)) < 0)
        goto ERROR_0;
    if (rpc_read(conn, &lda, sizeof(int)) < 0)
        goto ERROR_0;
        if (rpc_read(conn, (void*)B, sizeof(const float*)) < 0)
    if (rpc_read(conn, &B, sizeof(const float*)) < 0)
        goto ERROR_0;
    if (rpc_read(conn, &ldb, sizeof(int)) < 0)
        goto ERROR_0;
        if (rpc_read(conn, (void*)beta, sizeof(const float*)) < 0)
    if (rpc_read(conn, &beta, sizeof(const float*)) < 0)
        goto ERROR_0;
